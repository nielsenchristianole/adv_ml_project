...
class MaskedCouplingLayer(nn.Module):
    ...
    def forward(self, z):
        ...
        """
        # implementing the forward transformation eq 15 in exercise
        x = torch.mul(self.mask, z) + torch.mul((1-self.mask), 
                                                torch.mul(z, torch.exp(self.scale_net(torch.mul(self.mask, z))))
...
if __name__ == "__main__":
    ...
    # Generate the data
    if args.data == "mnist":
        train_loader = datasets.MNIST('data/', train=True, download=True, transform=transforms.Compose([
                        transforms.ToTensor(),
                        transforms.Lambda(lambda x: x + torch.rand(x.shape)/255), transforms.Lambda(lambda x: x.flatten())
                        ]))
        train_loader = torch.utils.data.DataLoader(train_loader, batch_size=args.batch_size, shuffle=True)
        test_loader = datasets.MNIST('data/', train=False, download=True, transform=transforms.Compose([
                        transforms.ToTensor(),
                        transforms.Lambda(lambda x: x + torch.rand(x.shape)/255), transforms.Lambda(lambda x: x.flatten())
                        ]) )
        test_loader = torch.utils.data.DataLoader(test_loader, batch_size=args.batch_size, shuffle=True)
    else:
        n_data = 10000000
